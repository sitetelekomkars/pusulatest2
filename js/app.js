...
function switchHubTab(tabId) {
    ...
    // YENİ: YETKİ KONTROLÜ - Sadece adminler Feedback/Form/Eğitim Atama butonlarını görsün
    const formStartBtn = document.getElementById('btn-start-eval');
    const feedbackSendBtn = document.getElementById('btn-send-feedback');
    const assignEduPanel = document.getElementById('admin-edu-panel');
    
    if(formStartBtn) formStartBtn.style.display = isAdminMode ? 'inline-block' : 'none';
    if(feedbackSendBtn) feedbackSendBtn.style.display = isAdminMode ? 'block' : 'none';
    if(assignEduPanel) assignEduPanel.style.display = isAdminMode ? 'block' : 'none';
}
...
async function fetchEvaluationsForAgent(forcedName) {
    ...
    if (isAdminMode && targetAgent === 'all' && targetGroup === 'all') {
        loader.innerHTML = '<div style="padding:20px; text-align:center;">Lütfen bir seçim yapın.</div>';
        return;
    }
...
async function logEvaluationPopup() {
    ...
    // DÜZELTME: Chat personeli için artık tek form var, pop-up sormadan direkt 'Chat' kullan.
    // Eski kodda burası pop-up ile soruluyordu, şimdi kaldırıldı.
    if (agentGroup.includes('Chat')) {
        agentGroup = 'Chat'; 
    }
    // Telesatış zaten "Telesatış" olarak kalmalı
    
    const isChat = (agentGroup === 'Chat');
    const isTelesatis = (agentGroup === 'Telesatış');
    ...
    if (isCriteriaBased) {
        ...
        criteriaList.forEach((c, i) => {
            ...
            // Eğer Chat ise Butonlu, değilse (Telesatış) Sliderlı yapı
            if (isChat) {
                // CHAT: Butonlu Puanlama (İyi/Orta/Kötü)
                // Kriterlerin orta/kötü puan değerlerini alalım, yoksa varsayılan yapalım
                let mPts = parseInt(c.mediumScore) || Math.floor(pts / 2);
                let bPts = parseInt(c.badScore) || 1;
                ...
                criteriaFieldsHtml += `
                 ...
                 // TELESATIŞ: Slider Puanlama (Eski mantık korundu)
                 criteriaFieldsHtml += `
                    <div class="criteria-row" id="row-${i}" data-max-score="${pts}">
                        <div class="criteria-header">
                            <span>${i+1}. ${c.text}</span>
    ...
    const { value: formValues } = await Swal.fire({
        ...
        preConfirm: () => {
            ...
            if (isCriteriaBased) {
                ...
                return { agentName, callId: evalData.callId, score: total, details: JSON.stringify(detailsArr), feedback };
            } else {
                // Manuel Puanlama
                const score = document.getElementById('eval-manual-score').value;
                const details = document.getElementById('eval-details').value;
