...
function renderCards(data) {
    try {
        ...
        const htmlChunks = data.map((item, index) => {
            const safeTitleJs = escapeForJsString(item.title); // Keep for JS onclick args
            const safeTitleHtml = escapeHtml(item.title);      // Use for display
            ...
            // SECURITY: Escape first, then add trusted HTML
            let formattedText = escapeHtml(item.text || "").replace(/\n/g, '<br>').replace(/\*(.*?)\*/g, '<b>$1</b>');
            const safeScript = escapeHtml(item.script);
            const safeCode = escapeHtml(item.code);
            const safeCategory = escapeHtml(item.category);

            return `<div class="card ${safeCategory}">${newBadge}
                <div class="icon-wrapper">${editIconHtml}<i class="${favClass} fav-icon" onclick="toggleFavorite('${safeTitleJs}')"></i></div>
                <div class="card-header"><h3 class="card-title">${highlightText(safeTitleHtml)}</h3><span class="badge">${safeCategory}</span></div>
                <div class="card-content" onclick="showCardDetail('${safeTitleJs}', '${escapeForJsString(item.text)}')">
                ...
                <div class="card-actions">
                    ...
                    ${item.link ? `<a href="${encodeURI(item.link)}" target="_blank" class="btn btn-link"><i class="fas fa-external-link-alt"></i> Link</a>` : ''}
                </div>
            </div>`;
        });
...
function openNews() {
    ...
    newsData.forEach((i, index) => {
        ...
        c.innerHTML += `<div class="news-item" style="${passiveStyle}">${editBtn}<span class="news-date">${escapeHtml(i.date)}</span><span class="news-title">${escapeHtml(i.title)} ${passiveBadge}</span><div class="news-desc">${escapeHtml(i.desc)}</div><span class="news-tag ${cl}">${tx}</span></div>`;
    });
}
        ...
        listEl.innerHTML += `
            <div class="feedback-card" style="border-left-color: ${feedbackClass};">
                <div class="feedback-header">
                    <div style="font-weight:bold; color:#0e1b42; font-size:1.1rem;">${escapeHtml(e.agent)}</div>
                    <div class="feedback-info-right">
                        <span><i class="fas fa-user-check"></i> Değerleyen: ${escapeHtml(e.evaluator)}</span>
                        <span><i class="fas fa-id-badge"></i> Çağrı ID: ${escapeHtml(cleanCallId)}</span>
                        <span><i class="fas fa-calendar-alt"></i> Tarih: ${escapeHtml(e.callDate)}</span>
                    </div>
                </div>
                <div class="feedback-body">
                    <div style="font-weight:bold; color:#333; margin-bottom:5px;">Konu/Açıklama: ${escapeHtml(feedbackTopic)}</div>
                    <div style="color:#555; line-height:1.5; font-size:0.95rem;">${escapeHtml(e.feedback)}</div>
                </div>
                <div class="feedback-footer">
                     <div style="display:flex; gap:10px; font-size:0.7rem; color:#666; font-weight:600; margin-right:10px;">
                        <span><i class="fas fa-calendar-week"></i> Dönem: ${escapeHtml(period)}</span>
                        <span><i class="fas fa-comment-alt"></i> Kanal: ${escapeHtml(channel)}</span>
                        <span><i class="fas fa-tag"></i> Tip: ${escapeHtml(infoType)}</span>
                     </div>
    ...
    try {
        ...
        if (data.result === "success") {
            ...
            normalEvaluations.forEach((evalItem, index) => {
                ...
                const safeCallId = escapeHtml(evalItem.callId || '');
                let editBtn = isAdminMode ? `<i class="fas fa-pen" style="font-size:1rem; color:#fabb00; cursor:pointer; margin-right:5px;" onclick="event.stopPropagation(); editEvaluation('${escapeForJsString(evalItem.callId)}')"></i>` : '';
                ...
                try {
                    ...
                    if (Array.isArray(detailObj)) {
                        detailObj.forEach(item => {
                            ...
                            let noteDisplay = item.note ? `<br><em style="color: #d32f2f; font-size:0.8rem;">(Not: ${escapeHtml(item.note)})</em>` : '';
                            detailHtml += `<tr style="background:${rowColor}; border-bottom:1px solid #fff;">
                                <td style="padding:8px; border-radius:4px;">${escapeHtml(item.q)}${noteDisplay}</td>
                                <td style="padding:8px; font-weight:bold; text-align:right;">${item.score}/${item.max}</td>
                            </tr>`;
                        });
                    } else {
                        detailHtml = `<p style="white-space:pre-wrap; margin:0; font-size:0.9rem; background:#fff8e1; padding:10px; border-radius:4px;">${escapeHtml(evalItem.details)}</p>`;
                    }
                    detailHtml += '</table>';
                } catch (e) {
                    detailHtml = `<p style="white-space:pre-wrap; margin:0; font-size:0.9rem; background:#fff8e1; padding:10px; border-radius:4px;">${escapeHtml(evalItem.details)}</p>`;
                ...
                const callDateDisplay = evalItem.callDate && evalItem.callDate !== 'N/A' ? escapeHtml(evalItem.callDate) : 'N/A';
                const listenDateDisplay = evalItem.date || 'N/A'; // date formatlı gelir genelde
                ...
                <div class="evaluation-summary" id="eval-summary-${index}" style="border-left:4px solid ${scoreColor}; padding:15px; margin-bottom:10px; border-radius:8px; background:#fff; cursor:pointer;" onclick="toggleEvaluationDetail(${index})">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <div>
                            ...
                            <div class="eval-date-info">
                                ...
                                <span><i class="fas fa-headphones"></i> Dinlenme: ${escapeHtml(listenDateDisplay)}</span>
                            </div>
                            <div style="margin-top:5px;">
                                <span class="eval-id-badge" onclick="event.stopPropagation(); copyText('${escapeForJsString(safeCallId)}')" title="Kopyala"><i class="fas fa-copy"></i> ${safeCallId}</span>
                            </div>
                        </div>
                    ...
                    <div class="evaluation-details-content" id="eval-details-${index}">
                        ...
                        <div style="margin-top:10px; background:#f8f9fa; padding:10px; border-radius:4px;">
                            <strong>Feedback:</strong> ${escapeHtml(evalItem.feedback || '-')}
                        </div>
                    </div>
                </div>`;
            });
