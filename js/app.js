...
// İstatistik kartlarını sıfırlama
const dashElements = ['dash-avg-score', 'dash-eval-count', 'dash-target-rate', 'dash-negative-notes', 'dash-revision-count'];
dashElements.forEach(id => {
const el = document.getElementById(id);
if (el) el.innerText = id === 'dash-target-rate' ? '-%' : '-';
});

// Admin filtresi ve veri çekme
if (isAdminMode) {
...
// YENİ EKLENEN DASHBOARD ELEMENTLERİ
const dashNegativeNotes = document.getElementById('dash-negative-notes');
const dashRevisionCount = document.getElementById('dash-revision-count');

listEl.innerHTML = '';
loader.style.display = 'block';

// Dashboard elementlerini resetleme
[dashAvg, dashCount, dashTarget, dashNegativeNotes, dashRevisionCount].forEach(el => {
if (el) el.innerText = (el.id === 'dash-target-rate' ? '-%' : '-');
});
...
html += `<div class="evaluation-summary" id="eval-summary-${index}" style="position:relative; border:1px solid #eaedf2; border-left:4px solid ${scoreColor}; padding:15px; margin-bottom:10px; border-radius:8px; background:#fff; cursor:pointer; transition:all 0.2s ease;"
